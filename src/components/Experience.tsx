    ...
    <section id="experience" className="py-24 md:py-32 bg-charcoal dark:bg-charcoal-light text-cream transition-colors duration-300">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-[5px] md:left-1/2 top-0 bottom-0 w-px bg-cream/20 md:-translate-x-px" />
...
function ExperienceCard({
  ...
  return (
    <div className={`relative flex items-start ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
      {/* Timeline dot */}
      <div className="absolute left-0 md:left-1/2 top-8 md:top-1/2 w-3 h-3 bg-accent rounded-full md:-translate-x-1.5 md:-translate-y-1/2 z-10 ring-4 ring-charcoal dark:ring-charcoal-light" />
      ...
      <div className={`w-full md:w-1/2 pl-8 md:pl-0 ${isEven ? 'md:pr-16' : 'md:pl-16'}`}>
        <div className="group relative p-6 md:p-8 rounded-2xl bg-charcoal-light/50 border border-cream/10 hover:border-accent/30 transition-all">
          ...
          {/* Date badge (mobile) */}
          <div className="md:hidden text-sm text-cream/50 mb-2">
            {experience.startDate} — {experience.endDate}
          </div>
